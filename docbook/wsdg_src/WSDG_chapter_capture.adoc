...
Extcaps may be any binary or script within the extcap directory. Please note
that scripts must be executable (`chmod +x some.py`) and start with an
appropriate “shebang” line at the beginning of its script (for example,
`#!/usr/bin/env python3`).

On Windows, the file extension must be present in the `PATHEXT` environment
variable and the default file association must be set to the interpreter.
For Python scripts, you must enable “Install for all users” and “Add Python to
environment variables” in the Python installer for this to happen.
If you are not able to get this to work, you may also create a batch file that
executes the script.
...
Scripts are only executed by Wireshark if their extension is present in the
`PATHEXT` environment variable, and a file association is correctly configured.
If you cannot satisfy this requirement, you can workaround this using a batch
file that calls the original script.

For example, if the Python script is named `sampleif.py`, create `sampleif.bat`
in the same extcap directory with the following contents:
...
Replace `python` by the full path if `python.exe` is not in your `PATH`
environment. `%~dpn0` expands to the concatenation of the drive letter, the
path, and the file name (without extension) of the invoked batch file.
