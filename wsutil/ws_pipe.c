 ...
 */
static gchar **
convert_to_argv(const char *command, int argc, char *const *args)
{
    gchar **argv = g_new(gchar *, argc + 2);
    ...
    for (int i = 0; i < argc; i++) {
        // XXX remove this? The caller violated contract.
        if (!args[i]) {
...
GPid ws_pipe_spawn_async(ws_pipe_t *ws_pipe, GPtrArray *args)
{
    ...
    gchar **argv = convert_to_argv(args_array[0], args->len - 1, args_array + 1);
    gchar *command_line = convert_to_command_line(argv);
